package com.example.administrator.signaturetest.Display_pictures_drag;
...
import com.example.administrator.signaturetest.MainActivity;
import com.example.administrator.signaturetest.MyImage.ScaleImageView;
...
public class Display_pictures_drag extends AppCompatActivity implements MyReceiver.BRInteraction  {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        final View view = View.inflate(this, R.layout.scaleimage, null);
        //隐藏状态栏
        ...
        ScaleImageView scaleImageView = (ScaleImageView) findViewById(R.id.image2);
        ...
        wv_Change = (WebView) findViewById(R.id.checkVersion);
        wv_Change.setWebViewClient(new WebViewClient() {
        ...
        wv_Change.setWebChromeClient(new WebChromeClient() {
            //处理alert弹出框
            @Override
        ...
        wv_Change.getSettings().setJavaScriptEnabled(true);  //设置WebView属性,运行执行js脚本
        wv_Change.getSettings().setUseWideViewPort(true);
        wv_Change.getSettings().setLoadWithOverviewMode(true);
        wv_Change.getSettings().setJavaScriptEnabled(true);
        wv_Change.addJavascriptInterface(this,"callByJs");
        //下载APK
        ...
        if (isNet) {
            wv_Change.loadUrl("http://59.110.226.225/Detectionversion.view?PhoneSys=Android");//调用loadView方法为WebView加入链接
        } else {
...
//        Toast.makeText(this,"页面调用原生方法",Toast.LENGTH_SHORT).show();
        if (Double.parseDouble(version) <= Double.parseDouble(getVersion())) {
            ...
            Log.d("apk", apk);
            Log.d("apk完整地址",  "http://59.110.226.225/upload/" + apk);
            isSame = true;
            //监听动画运行过程
...
//            redirectTo();
            ...
            isSame = false;
            Apk_Url = "http://59.110.226.225/upload/" + apk;
            //检测读写权限方法
    ...
    protected void onStart() {
        ...
        if (isNet){
            wv_Change.loadUrl("http://123.56.187.157/hlts/Detectionversion.view?PhoneSys=Android");//调用loadView方法为WebView加入链接
        }else{
            Toast.makeText(this,"请检查您的网络",Toast.LENGTH_LONG).show();
        }*/
    }
